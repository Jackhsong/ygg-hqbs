...
<html>
	...
	<body>
				     ...
				     <select id="province" name="province">
						...
						<#if provinceList?exists && (provinceList?size>0) >
						  <#list  provinceList as pl >
						      <#if address?? && address.province?? && (address.province) ==(pl.provinceId) >
				...
				<li><span>城市</span>
				    <select id="city"   name="city" >
						...
						<#if cityList?exists && (cityList?size>0) >
						  <#list  cityList as cl >
						     <#if address?? && address.city?? &&  (address.city) ==(cl.cityId) >
				     ...
				     <select id="region"  name="district" >
						...
						<#if districtList?exists && (districtList?size>0) >
						  <#list  districtList as dl >
						      <#if address??   && address.district?? &&  (address.district) ==(dl.districtId) >
		...
		function sendform(){
		    ...
		    if(checkflag){
		            if(!flag){
		        		document.form1.action="${rc.contextPath}/ads/addads";
