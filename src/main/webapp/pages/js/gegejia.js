...
$(function(){	
	...
	$('.payta').click(function(){
		...
		$(this).find('.cpayflag').attr('src','/ygg-hqbs/pages/images/select.png');
		$('.paytb').find('.cpayflag').attr('src','/ygg-hqbs/pages/images/unselect.png');
		$('.paytc').find('.cpayflag').attr('src','/ygg-hqbs/pages/images/unselect.png');
	...
	$('.paytb').click(function(){
		...
		$(this).find('.cpayflag').attr('src','/ygg-hqbs/pages/images/select.png');
		$('.payta').find('.cpayflag').attr('src','/ygg-hqbs/pages/images/unselect.png');
		$('.paytc').find('.cpayflag').attr('src','/ygg-hqbs/pages/images/unselect.png');
...
function browserRedirect() {  
	var carSH = $("#footBar").height();
	$('.buy').animate({bottom:0});
	$('.carttotal').css({bottom:carSH-1});
    ...
    if (!(bIsIpad || bIsIphoneOs || bIsMidp || bIsUc7 || bIsUc || bIsAndroid || bIsCE || bIsWM) ){  
        ...
        $('.pagea').removeClass('pagea').css({'width':'640px','height':'100%','margin':'0 auto'});
        $('.pagep').removeClass('pagep').addClass('wpagep');
        $('.swiper-slide img').css('width','640');
        $('.buy').css({'left':'50%','marginLeft':'-320px'});
...
// 获取用户来源不是站内进来的隐藏返回首页的模块
function refUlr(){
	var ref = '';  	
	var inUrl = "wx.globalscanner.com";
	var testUrl = "test.gegejia.com";
	...
	if(ref.indexOf(inUrl) < 0  && ref.indexOf(testUrl) < 0){
		$(".top-back-home").css({top:0}).parent().addClass("pt-54");
	}
}
