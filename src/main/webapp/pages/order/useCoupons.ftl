...
.pronotice img{ position: absolute; top: 22px; left: 50%; width: 20px; margin-left: -10px}
	...
	<body>
		<div class="page">
			...
			<a href="${rc.contextPath}/spcart/listsc" class="tit_ra"><img src="${rc.contextPath}/pages/images/cart.png" class="tit_cart"></a>				
			</div>
           ...
           <#if addressId?exists && addressId!='' &&addressId !='-1' >
				...
				<#if confirmOrderList?exists && (confirmOrderList?size>0 ) >
				  <#list confirmOrderList as cl>
						<a href="javascript:void(0)">
							<div class="order">
								<div class="order_tit clear"><span>订单${(cl_index+1)}</span><span>${(cl.sellerName)!""}${(cl.sendAddress)!""}发货 <#if (cl.ismailbag)?exists && (cl.ismailbag) =='0'>(包邮)</#if></span></div>
								<!-- 订单商品列表 -->
			...
			<div class="paytype">
				<div class="choosepay">优惠减免</div>
				<div class="payta"><i class="fl">优惠券</i><a href="" class="fr">使用优惠券</a></div>
				<div class="paytb">积分底现<i class="usePoint">使用1000积分抵<b>￥100</b></i><em class="pointBtn open"></em><span></div>
				<div class="paytc"><img src="${rc.contextPath}/pages/images/ylpay.png" class="cpay">银联支付<span><img src="${rc.contextPath}/pages/images/unselect.png" class="cpayflag"></span></div>
			</div>
			<div class="paytype">
				<div class="choosepay">选择支付方式<span>合计金额：<em>${allTotalPrice!"0.00"}</em></span></div>
				<div class="paytb" id="paywexin"><img src="${rc.contextPath}/pages/images/wxpay.png" class="cpay">微信支付<span><img src="${rc.contextPath}/pages/images/unselect.png" class="cpayflag"></span></div>
			</div>
			<div class="pay clear">
				<ul>
					<li>支付宝支付</li>
				...
				总计￥<span>${allTotalPrice!"0.00"}</span>
		...
		<input type="hidden" id="totalPrice"  name="totalPrice"   value="${allTotalPrice!"0.00"}"/>
		</form>
