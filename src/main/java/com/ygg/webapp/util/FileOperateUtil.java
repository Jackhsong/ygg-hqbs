...
public class FileOperateUtil
{
    ...
    public static String downloadDir = "";// 在web.xml中指定
    
    public static int dtimes = 0;
    
    static Logger logger = Logger.getLogger(FileOperateUtil.class);
     ...
     * @throws Exception
     */
    public static List<Map<String, Object>> upload(HttpServletRequest request, String[] params, Map<String, Object[]> values)
        throws Exception
    {
        ...
        for (Iterator<Map.Entry<String, MultipartFile>> it = fileMap.entrySet().iterator(); it.hasNext(); i++)
        {
            ...
            // 自定义参数值对
            /*
             * if(params !=null && params.length>0) for (String param : params) { map.put(param, values.get(param)[i]);
             * }
             */
            result.add(map);
        }
        ...
        throws Exception
    {
        ...
        downLoadPath = downloadDir + storeName; // "E:\\YggAndroid.apk" ;
        logger.info("----downLoadPath----- " + downLoadPath + "download times is: " + dtimes);
