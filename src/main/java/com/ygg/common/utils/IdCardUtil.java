...
public class IdCardUtil
{
    ...
    public static Logger log = Logger.getLogger(IdCardUtil.class);
    ...
    private static String[] _areaCode = {"11", "12", "13", "14", "15", "21", "22", "23", "31", "32", "33", "34", "35", "36", "37", "41", "42", "43", "44", "45", "46", "50", "51",
    ...
    private static boolean verifyAreaCode(String code)
    {
        ...
        {
            log.warn("错误：输入的身份证号的地区码(1-2位)[" + areaCode + "]不符合中国行政区划分代码规定(GB/T2260-1999)");
            return false;
        }
    }
    ...
    private static boolean verifyBirthdayCode(String code)
    {
        ...
        {
            log.warn("错误：输入的身份证号" + (isEighteenCode ? "(11-12位)" : "(9-10位)") + "不存在[" + month + "]月份,不符合要求(GB/T7408)");
            return false;
        }
        // 验证日期
        ...
        if (day != null)
        {
            ...
            {
                log.warn("错误：输入的身份证号" + (isEighteenCode ? "(13-14位)" : "(11-13位)") + "[" + dayCode + "]号不符合小月1-30天大月1-31天的规定(GB/T7408)");
                return false;
            }
        }
        // 2月的情况
        else
        {
            ...
            if ((year % 4 == 0 && year % 100 != 0) || (year % 400 == 0))
            {
                ...
                {
                    log.warn("错误：输入的身份证号" + (isEighteenCode ? "(13-14位)" : "(11-13位)") + "[" + dayCode + "]号在" + year + "闰年的情况下未符合1-29号的规定(GB/T7408)");
                    return false;
                }
            }
            // 非闰月的情况
            else
            {
                ...
                {
                    log.warn("错误：输入的身份证号" + (isEighteenCode ? "(13-14位)" : "(11-13位)") + "[" + dayCode + "]号在" + year + "平年的情况下未符合1-28号的规定(GB/T7408)");
                    return false;
                }
            }
        }
    ...
    private static boolean containsAllNumber(String code)
    {
        ...
        for (int i = 0; i < ch.length; i++)
        {
            if (!(ch[i] >= '0' && ch[i] <= '9'))
            {
                log.warn("错误：输入的身份证号第" + (i + 1) + "位包含字母");
                return false;
            }
        }
        return true;
    ...
    private static boolean verifyMOD(String code)
    {
        ...
        // x=14;
        // }
        log.warn("错误：输入的身份证号最末尾的数字验证码错误");
        return false;
    }
